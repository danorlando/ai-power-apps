...
export const getAvatar = ({ size=30, sender, isCreatedByUser, searchResult, platform, chatGptLabel, error, className = '', isButton = false}: TAvatarProps) => {
  // 'ai' is used as 'model' is not accurate for search results
  let ai = searchResult ? sender : platform;
  const bgColors = {
  ...
  if (isCreatedByUser) 
    return (
      <div
        ...
        className={`relative flex items-center justify-center rounded-sm text-white ` + className}
      >
        User
      </div>
    )
    ...
    return (
      <div
        title='User'
        style={{ background: 'radial-gradient(circle at 90% 110%, rgb(1 43 128), rgb(17, 139, 161))', color: 'white', fontSize: 12, width: size, height: size }}
        className={`relative flex items-center justify-center rounded-sm p-1 text-white ` + className}
      >
        {chatGptLabel}
      </div>
    )
